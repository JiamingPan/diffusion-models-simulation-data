...
class DitL16ContinuationTests(unittest.TestCase):
    ...
    def test_slurm_workflow_is_staged_and_samples_exact_checkpoint(self):
        ...
        precheck = (REPO_ROOT / "scripts" / "slurm" / "precheck_nf_generalize_fig2_dit_l16_resume.sbatch").read_text()
        ...
        self.assertIn("PREVIOUS_EXPECTED_CHECKPOINT=$(field previous_expected_checkpoint)", train)
        self.assertIn('--checkpoint-dir "${CHECKPOINT_DIR}"', train)
        self.assertIn('--minimum-checkpoint "${PREVIOUS_EXPECTED_CHECKPOINT}"', train)
        self.assertIn('--target-checkpoint "${EXPECTED_CHECKPOINT}"', train)
        self.assertIn("expected_checkpoint", sample)
        self.assertIn('--checkpoint "${CHECKPOINT_PATH}"', sample)
        self.assertIn("--print-field previous_expected_checkpoint", precheck)
        self.assertNotIn("find \"${CHECKPOINT_DIR}\"", precheck)
        ...
        self.assertIn("--seed-checkpoints", submit)
