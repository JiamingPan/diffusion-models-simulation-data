...
TARGET_DIR=${TARGET_DIR:-/home/jiamingp/Diffusion_model/cosmo_diffusion_main}
...
from cosmodiff.utils import parse_config_data
import ema_pytorch  # noqa: F401
...
required_generate = {"num_steps", "s_churn", "s_tmin", "s_tmax", "s_noise"}
missing_generate = required_generate.difference(inspect.signature(optim.generate).parameters)
if missing_generate:
    raise SystemExit(f"cosmodiff.optim.generate is too old; missing {sorted(missing_generate)}")
...
print("parse_config_data:", inspect.getsourcefile(parse_config_data))
print("Normalization:", Normalization)
print("Transform:", Transform)
PY

echo
echo "Use this checkout for nf_sweep_v2 jobs:"
