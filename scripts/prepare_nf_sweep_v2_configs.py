#!/usr/bin/env python
"""Prepare v2 n=500 sweep configs using merged cosmo_diffusion main defaults.

This is intentionally separate from ``nf_sweep`` so already-running jobs remain
comparable.  The v2 sweep follows Nicholas Kern's merged default config:

- ``data.reshape: '2d'`` instead of ``two_dim: true``
- ``data.transform: [log]`` instead of ``log: true``
- cosine DDPM schedule with zero-terminal-SNR rescaling
- ``v_prediction`` + Min-SNR loss weighting
- post-hoc EMA with burn-in
- new ``generate:`` block for fast inference schedulers
"""
...
CHECKPOINT_ROOT = "/scratch/huterer_root/huterer0/jiamingp/saved_runs/nf_sweep_v2"
...
VARIANTS = [
    {
        ...
        "ema_burn_in": 1000,
        "note": "Exact main-branch default training recipe except width.",
    },
    {
        "tag": "sigmoid_zero_snr",
        "label": "sigmoid + zero-SNR + Min-SNR",
        ...
        "sigma_log_normal": None,
        "ema_burn_in": 1000,
        "note": "Keeps new Min-SNR/EMA defaults but compares sigmoid to cosine beta schedule.",
    },
    {
        "tag": "cosine_no_zero_snr",
        "label": "cosine without zero-SNR",
        "beta_schedule": "squaredcos_cap_v2",
        ...
        "sigma_log_normal": None,
        "ema_burn_in": 1000,
        "note": "Ablates zero-terminal-SNR rescaling from Nicholas default.",
    },
    {
        "tag": "no_minsnr",
        "label": "Nicholas default without Min-SNR",
        "beta_schedule": "squaredcos_cap_v2",
        "rescale_betas_zero_snr": True,
        "prediction_type": "v_prediction",
        "min_snr_gamma": None,
        "sigma_log_normal": None,
        "ema_burn_in": 1000,
        "note": "Ablates Min-SNR loss weighting.",
    },
    {
        "tag": "no_ema_burnin",
        "label": "Nicholas default without EMA burn-in",
        "beta_schedule": "squaredcos_cap_v2",
        "rescale_betas_zero_snr": True,
        "prediction_type": "v_prediction",
        "min_snr_gamma": 5.0,
        "sigma_log_normal": None,
        "ema_burn_in": 0,
        "note": "Ablates the first-1000-step EMA burn-in.",
    },
    {
        "tag": "sigma_edm",
        "label": "Nicholas default + EDM log-normal sigma",
        "beta_schedule": "squaredcos_cap_v2",
        "rescale_betas_zero_snr": True,
        "prediction_type": "v_prediction",
        "min_snr_gamma": 5.0,
        ...
        "ema_burn_in": 1000,
        "note": "Adds EDM default log-sigma timestep sampling.",
    },
    {
        "tag": "sigma_edm2",
        "label": "Nicholas default + EDM2 latent log-normal sigma",
        "beta_schedule": "squaredcos_cap_v2",
        "rescale_betas_zero_snr": True,
        "prediction_type": "v_prediction",
        "min_snr_gamma": 5.0,
        ...
        "ema_burn_in": 1000,
        "note": "Adds EDM2 latent-image log-sigma timestep sampling.",
    },
]


def run_name(arch: str, tag: str) -> str:
    return f"nf_sweep_v2_{arch}_n500_e100_{tag}"
...
def build_config(name: str, arch: str, variant: dict[str, Any]) -> dict[str, Any]:
    ...
    scheduler_kwargs = {
        ...
        "rescale_betas_zero_snr": variant["rescale_betas_zero_snr"],
        "prediction_type": variant["prediction_type"],
        "clip_sample": False,
        "thresholding": False,
    ...
    return {
        ...
        "train": {
            ...
            "ema_update_every": 1,
            "ema_burn_in": variant["ema_burn_in"],
            "min_snr_gamma": variant["min_snr_gamma"],
    ...
    for arch, arch_cfg in ARCHES.items():
        for variant in VARIANTS:
            ...
            rows.append({
                ...
                "ema_sigma_rels": EMA_SIGMA_RELS,
                "ema_burn_in": variant["ema_burn_in"],
                "config": f"local/nf_sweep_v2/configs/{name}.yaml",
...
def assert_config(path: Path, expected: dict[str, Any]) -> None:
    ...
    checks = {
        ...
        "train.ema_burn_in": config["train"].get("ema_burn_in") == expected["train"]["ema_burn_in"],
        "train.min_snr_gamma": config["train"].get("min_snr_gamma") == expected["train"]["min_snr_gamma"],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--print-runs", action="store_true", help="Print run names and exit.")
...
def main() -> None:
    ...
    config_dir = project_dir / "local" / "nf_sweep_v2" / "configs"
    ...
    if args.check_only:
        ...
        print(f"Validated {len(selected)} nf_sweep_v2 configs.")
        return
