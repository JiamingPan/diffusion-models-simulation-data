#!/usr/bin/env python
"""Prepare n=500 normalization-fix hyperparameter sweep configs.

This sweep starts from the already-run ``nf_u64_n500_e100`` and
``nf_u128_n500_e100`` setup, then changes one diffusion-training knob at a
time.  All new runs enable post-hoc EMA so that EMA length can be swept during
...
CHECKPOINT_ROOT = "/scratch/huterer_root/huterer0/jiamingp/saved_runs/nf_sweep"
...
def run_name(arch: str, tag: str) -> str:
    return f"nf_sweep_{arch}_n500_e100_{tag}"
            ...
            rows.append(
                {
                    ...
                    "min_snr_gamma": variant["min_snr_gamma"],
                    "ema_sigma_rels": EMA_SIGMA_RELS,
                    "config": f"local/nf_sweep/configs/{name}.yaml",
...
def assert_config(path: Path, expected: dict[str, Any]) -> None:
    ...
    checks = {
        ...
        "train.ema_burn_in": config["train"].get("ema_burn_in") == 0,
        ...
        "no_generate_block": "generate" not in config,
    }
...
def main() -> None:
    ...
    if args.check_only:
        ...
        print(f"Validated {len(selected)} nf_sweep configs.")
        return
