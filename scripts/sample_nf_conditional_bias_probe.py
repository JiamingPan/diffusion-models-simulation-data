...
def output_path_for(project_dir: Path, row: dict[str, Any], seed: int, k: int) -> Path:
    ...
    return project_dir / raw.format(seed=seed, sample_label="dpm50", k=k)


def annotate_npz(path: Path, row: dict[str, Any], seed: int, k: int, project_dir: Path) -> None:
    ...
    heldout_raw = np.load(project_dir / "local" / SWEEP_NAME / "heldout" / "heldout_params_raw.npy").astype(np.float32)
    payload.update(
        {
...
def main() -> None:
    ...
    for row in rows:
        ...
        output_path = output_path_for(project_dir, row, args.seed, k)
        ...
        if rc != 0:
            raise SystemExit(rc)
        annotate_npz(output_path, row, args.seed, k, project_dir)
