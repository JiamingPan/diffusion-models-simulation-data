...
\vspace{0pt}
For inference, a diffusion generator becomes a prior over HI fields $x$. The
cartoon shows the failure mode: orange is the true field distribution $p(x)$,
black dots are training maps, purple $s_\theta(x)$ is the learned diffusion
score/prior, and blue is the data constraint $y$. If $s_\theta(x)$ wraps only
training islands, $p(x|y)$ can be sharp but biased. We therefore test:

\begin{enumerate}[leftmargin=9mm, itemsep=1.8mm, label=\textcolor{paramBlue}{\textbf{\arabic*.}}]
\item \hl{loopRed}{Novelty:} when do generated maps stop matching individual
      training slices?
...
\vspace{3mm}
\centering
\figslot{15.0cm}{13.2cm}{inference_manifold_implication.pdf}
...
\SmallFont
For each generated HI map $x_j$, compare its image or feature vector to every
training map $y_i$ and keep the closest match:
\begingroup
...
\]
\endgroup
A high $s_j$ means $x_j$ is nearly a copy of one training map. Here
$\mathrm{sim}$ is cosine similarity (larger = more alike) measured in pixel,
PCA, and SSCD spaces; the panel labels show the pixel-space closest-match
``cos''. Reproducibility curves then check whether independent runs match the
same distribution without repeating the same slices.

\vspace{2mm}
\figslot{30.0cm}{13.0cm}{nf_generalize_fig2_u64_generated_vs_pixel_nn.png}
...
\item The jump happens over a narrow range of $N_{2D}$; in these runs the
      UNet-256 transition appears at larger $N_{2D}$
      {\color{subgray}(Zhang et al.\ 2024; Kadkhodaie et al.\ 2024; Yoon et
      al.\ 2023)}.
\end{itemize}
\end{tcolorbox}
...
is biased. The slope is measured through a finite-accuracy VGG encoder
($R^2=0.91$ on real held-out $\Omega_m$), so 0.26 vs 0.79 is a
probe-calibrated summary; uncertainty calibration is the main message.
\end{tcolorbox}
