...
For a held-out sample, request maps at a chosen cosmology
$\bm{\theta}_{\rm ask}$.  A \textbf{frozen encoder} then estimates
$\bm{\theta}_{\rm rec}$ from the generated maps.  If conditioning is working,
$\bm{\theta}_{\rm rec}$ should move with the request, not stay near the
...
$\bm{\theta}_{\rm ask}$.  In the high-data regime, different noise seeds give
a real spread and the intervals often bracket the requested $\Omega_m$.
Under memorization, many seeds collapse toward the same training-like fields:
the bars can look tight while still being biased.  This is why calibration
matters more than just small scatter.
\end{tcolorbox}
...
\item \textbf{Copying is measurable.} Nearest-neighbor and reproducibility
      tests expose when generated maps are training-set-like.
\item \textbf{More data changes behavior.} Increasing $N_{2D}$ moves models
      from memorized slices toward new fields with realistic statistics.
\item \textbf{Calibration is the test.} Useful emulators must be novel,
      statistically faithful, and faithful to the requested cosmology.
\end{enumerate}
\end{tcolorbox}
